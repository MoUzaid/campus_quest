...
//               <FaChevronRight className="breadcrumb-icon" aria-hidden="true" />
//             </li>
//             <li className="breadcrumb-item">
//               <a href="#" className="breadcrumb-link">
//                 Quizzes
...
//         <h1 className="card-title">{data.quiz.title || "Quiz Analytics"}</h1>
//         <p className="card-subtitle">
//           <FaChartLine className="subtitle-icon" />
//           Comprehensive Performance Analytics & Insights Dashboard
//         </p>
//       </div>
//     </div>
...
//        {/* In your section-header div, replace the action buttons with: */}
...
//     <button className="btn-header" aria-label="Export CSV">
//       <FaDownload className="btn-icon" />
//       Export CSV
//       <span className="btn-hint">Download</span>
//     </button>
    
//     <button className="btn-header btn-primary-header" aria-label="Refresh data">
...
//               <FaBook className="metadata-icon" />
//             </div>
//             <div className="metadata-content">
//               <span className="metadata-label">Subject</span>
//               <span className="metadata-value">{data.quiz.subject || "General"}</span>
//             </div>
//           </div>

//           <div className="metadata-card">
//             <div className="metadata-icon-wrapper">
//               <FaTrophy className="metadata-icon" />
//             </div>
//             <div className="metadata-content">
//               <span className="metadata-label">Total Marks</span>
//               <span className="metadata-value highlight">{data.quiz.totalMarks || 100}</span>
//             </div>
//           </div>

//           <div className="metadata-card">
//             <div className="metadata-icon-wrapper">
//               <FaFlag className="metadata-icon" />
//             </div>
//             <div className="metadata-content">
//               <span className="metadata-label">Passing Marks</span>
//               <span className="metadata-value">
//                 {data.quiz.passingMarks || 40}
...
//             </div>
//             <div className="metadata-content">
//               <span className="metadata-label">Total Attempts</span>
//               <span className="metadata-value">{data.quiz.totalAttempts || 0}</span>
//             </div>
//           </div>
...
//               <span className="metadata-value">
//                 {new Date(data.quiz.createdAt).toLocaleDateString() || "Not specified"}
//               </span>
//             </div>
//           </div>
//         </div>
...
import {
  ...
  FaAward, FaChartArea, FaCaretUp, FaCaretDown,
  FaChevronDown, FaExternalLinkAlt, FaListOl,
  FaInfoCircle, FaFlag, FaBook, FaSync, FaShareAlt
} from "react-icons/fa";
import { FiBarChart2, FiTrendingUp } from "react-icons/fi";
import { GiDuration, GiLaurelsTrophy } from "react-icons/gi";
import "./QuizAttemptedStudent.css";
...
const QuizAttemptedStudents = () => {
  ...
  const handleTopPerformersSort = useCallback((key) => {
    setTopPerformersSort(prev => ({
      key,
      direction: prev.key === key && prev.direction === "asc" ? "desc" : "asc"
    }));
  }, []);
  ...
  const getPerformanceLabel = useCallback((percentage) => {
    if (percentage >= 90) return "Excellent";
    if (percentage >= 75) return "Good";
    if (percentage >= 50) return "Average";
    if (percentage >= 25) return "Poor";
  ...
  // Memoized sorted students for better performance with large datasets
  const sortedStudents = useMemo(() => {
    if (!data?.attemptedStudents) return [];
    
    return [...data.attemptedStudents].sort((a, b) => {
      const multiplier = sortConfig.direction === "asc" ? 1 : -1;
  ...
  }, [data?.attemptedStudents, sortConfig]);
  ...
  const filteredStudents = useMemo(() => {
    ...
    return sortedStudents.filter(student => {
      ...
      const matchesSearch = 
        ...
        student.student.enrollmentNumber.toLowerCase().includes(searchLower) ||
        (student.student.email && student.student.email.toLowerCase().includes(searchLower));
  ...
  }, [sortedStudents, searchTerm, filterStatus]);

  // Memoized top 5 performers
  const sortedTopPerformers = useMemo(() => {
    if (!data?.attemptedStudents) return [];
    
    const sorted = [...data.attemptedStudents].sort((a, b) => {
      const multiplier = topPerformersSort.direction === "asc" ? 1 : -1;
      const key = topPerformersSort.key;
  ...
  }, [data?.attemptedStudents, topPerformersSort]);

  // Calculate stats only once
  const stats = useMemo(() => {
    if (!data) return null;
    
    const passedCount = data.attemptedStudents?.filter(s => s.passed).length || 0;
    const totalCount = data.attemptedCount || 1;
    
    return {
      passedCount,
      passingRate: (passedCount / totalCount) * 100,
      top5AvgScore: sortedTopPerformers.reduce((sum, student) => sum + student.score, 0) / Math.min(5, sortedTopPerformers.length) || 0
    };
  }, [data, sortedTopPerformers]);
    ...
    return (
      <div className="error-container">
        <div className="error-content">
          ...
          <p>{error}</p>
          <button onClick={fetchAttemptedStudents} className="btn-retry">
            <FaSync /> Retry
          </button>
        </div>
      </div>
    ...
    <div className="analytics-container">
          ...
          <div className="breadcrumb-nav">
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb-list">
                ...
                <li className="breadcrumb-item">
                  <a href="#" className="breadcrumb-link">Quizzes</a>
                </li>
                <li className="breadcrumb-divider">
                  <FaChevronRight className="breadcrumb-icon" />
                </li>
          ...
          <div className="metadata-section">
            <div className="section-header">
              ...
              <div className="action-buttons-header">
                ...
                <button 
                  ...
                  onClick={fetchAttemptedStudents}
            ...
            <div className="metadata-grid">
              ...
              <div className="metadata-card">
                ...
                <div className="metadata-content">
                  <span className="metadata-label">Total Attempts</span>
                  <span className="metadata-value">{data.attemptedCount || 0}</span>
                </div>
              </div>
              ...
              <div className="metadata-card">
                ...
                <div className="metadata-content">
                  <span className="metadata-label">Created On</span>
                  <span className="metadata-value">
                    {data.quiz?.createdAt ? new Date(data.quiz.createdAt).toLocaleDateString() : "Not specified"}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card stat-success">
          ...
          <div className="stat-content">
            <h3>{data.analytics?.averageScore?.toFixed(1) || "0.0"}</h3>
            <p>Average Score</p>
            <span className="stat-trend">
              <FiTrendingUp /> {stats?.passingRate?.toFixed(1) || "0"}% pass rate
            </span>
          </div>
        </div>
        ...
        <div className="stat-card stat-warning">
          ...
          <div className="stat-content">
            <h3>{data.analytics?.highestScore || 0}</h3>
            <p>Highest Score</p>
            <span className="stat-trend">
              <FiTrendingUp /> Top performer
            </span>
        ...
        <div className="stat-card stat-info">
          ...
          <div className="stat-content">
            <h3>
              {data.analytics?.timeStats?.averageTime 
                ? `${Math.floor(data.analytics.timeStats.averageTime / 60)}:${(data.analytics.timeStats.averageTime % 60).toString().padStart(2, '0')}`
                : "0:00"
              }
            </h3>
            <p>Avg. Time Taken</p>
      ...
      <div className="tabs-container">
        ...
        <div className="tabs">
          ...
          <button
            ...
            onClick={() => setActiveTab("students")}
          >
            <FaUserGraduate /> All Students ({data.attemptedCount || 0})
          </button>
          <button
            className={`tab-btn ${activeTab === "topPerformers" ? "active" : ""}`}
            onClick={() => setActiveTab("topPerformers")}
      ...
      <div className="tab-content">
          ...
          <div className="overview-grid">
            ...
            <div className="chart-card">
              <div className="chart-header">
                <h3><FaChartPie /> Performance Distribution</h3>
                <div className="chart-legend">
                  {data.analytics?.performanceDistribution && Object.entries(data.analytics.performanceDistribution).map(([key, value], index) => (
                    <div key={key} className="legend-item">
                      <span className="legend-color" style={{ backgroundColor: performanceColors[index] }}></span>
                      <span className="legend-label">{key.charAt(0).toUpperCase() + key.slice(1)}</span>
                      <span className="legend-value">({value})</span>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={data.analytics?.performanceDistribution ? 
                      Object.entries(data.analytics.performanceDistribution).map(([key, value], index) => ({
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        value,
                        color: performanceColors[index]
            ...
            <div className="chart-card">
              ...
              <div className="department-list">
                {data.chartData?.departmentDistribution?.map((dept, index) => (
                  <div key={dept.name || index} className="department-item">
                    <div className="dept-info">
                      <span className="dept-name">{dept.name || "Unknown"}</span>
                      <span className="dept-count">{dept.value || 0} students</span>
                    </div>
                    <div className="dept-bar-container">
                      <div 
                        className="dept-bar" 
                        style={{ 
                          width: `${((dept.value || 0) / (data.attemptedCount || 1)) * 100}%`,
                          backgroundColor: departmentColors[index % departmentColors.length]
                        }}
                      ></div>
                    </div>
                    <span className="dept-percentage">
                      {((dept.value || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="time-stats-grid">
                <div className="time-stat">
                  ...
                  <div className="time-stat-content">
                    <span className="time-label">Fastest</span>
                    <span className="time-value">
                      {data.analytics?.timeStats?.fastestTime 
                        ? `${Math.floor(data.analytics.timeStats.fastestTime / 60)}:${(data.analytics.timeStats.fastestTime % 60).toString().padStart(2, '0')}`
                        : "0:00"
                      }
                    </span>
                  </div>
                ...
                <div className="time-stat">
                  ...
                  <div className="time-stat-content">
                    <span className="time-label">Slowest</span>
                    <span className="time-value">
                      {data.analytics?.timeStats?.slowestTime 
                        ? `${Math.floor(data.analytics.timeStats.slowestTime / 60)}:${(data.analytics.timeStats.slowestTime % 60).toString().padStart(2, '0')}`
                        : "0:00"
                      }
                    </span>
                  </div>
        ...
        {activeTab === "topPerformers" && (
          <TopPerformersTab 
            sortedTopPerformers={sortedTopPerformers}
            data={data}
            topPerformersSort={topPerformersSort}
            handleTopPerformersSort={handleTopPerformersSort}
            setSelectedStudent={setSelectedStudent}
            getPerformanceColor={getPerformanceColor}
            getPerformanceLabel={getPerformanceLabel}
            stats={stats}
        ...
        {activeTab === "performance" && (
          <PerformanceTab 
            data={data}
            attemptedStudents={data.attemptedStudents}
            getPerformanceColor={getPerformanceColor}
  ...
  <div className="students-container">
    <div className="table-controls">
      <div className="controls-left">
        ...
        <div className="filter-buttons">
          <button 
            ...
            onClick={() => setFilterStatus("all")}
          >
            All ({data.attemptedCount || 0})
          </button>
          <button 
            className={`filter-btn ${filterStatus === "passed" ? "active" : ""}`}
            onClick={() => setFilterStatus("passed")}
          >
            <FaCheckCircle /> Passed ({data.attemptedStudents?.filter(s => s.passed).length || 0})
          </button>
          <button 
            className={`filter-btn ${filterStatus === "failed" ? "active" : ""}`}
            onClick={() => setFilterStatus("failed")}
          >
            <FaTimesCircle /> Failed ({data.attemptedStudents?.filter(s => !s.passed).length || 0})
          </button>
        </div>
      </div>
      ...
      <div className="controls-right">
        ...
        <div className="results-count">
          Showing {filteredStudents?.length || 0} of {data.attemptedCount || 0} students
        </div>
      </div>
    </div>
        ...
        <table className="students-table">
          <thead>
            <tr>
              ...
              <th className="student-col" onClick={() => handleSort("name")}>
                <div className="th-content">
                  Student
                  <FaSort className="sort-icon" />
                </div>
          ...
          <tbody>
            {filteredStudents?.map((student) => (
              <StudentRow 
                key={student.attemptId || student.student?.id}
                student={student}
                setSelectedStudent={setSelectedStudent}
                getPerformanceColor={getPerformanceColor}
                getPerformanceLabel={getPerformanceLabel}
                totalStudents={data.attemptedCount || 0}
              />
            ))}
          </tbody>
        </table>
...
const StudentRow = React.memo(({ student, setSelectedStudent, getPerformanceColor, getPerformanceLabel, totalStudents }) => {
  const percentage = parseFloat(student.percentage || 0);
  const performanceColor = getPerformanceColor(percentage);
  const performanceLabel = getPerformanceLabel(percentage);
  
  return (
    <tr className={student.passed ? "row-passed" : "row-failed"}>
      <td className="rank-cell">
        <div className={`rank-badge rank-${Math.min(student.rank || 1, 5)}`}>
          {student.rank <= 3 ? (
            <>
              <FaCrown className="crown-icon" />
              <span>#{student.rank}</span>
      ...
      <td className="academic-cell">
        <div className="academic-info">
          <div className="academic-item">
            <FaUniversity className="academic-icon" />
            <span>{student.student?.department || "N/A"}</span>
          </div>
          <div className="academic-item">
            <FaBookOpen className="academic-icon" />
            <span>{student.student?.course || "N/A"} - Sem {student.student?.semester || "N/A"}</span>
          </div>
          <div className="academic-item">
            <FaUserGraduate className="academic-icon" />
            <span>Group {student.student?.group || "N/A"}</span>
          </div>
        </div>
      </td>
      ...
      <td className="score-cell">
        <div className="score-display">
          <div className="score-main">
            <span className="score-value">{student.score || 0}</span>
            <span className="score-divider">/</span>
            <span className="score-total">{student.totalMarks || 100}</span>
          </div>
          <div className="score-progress">
            <div className="score-bar">
              <div 
            ...
            <div className="score-labels">
              <span className="passing-label">Pass: {student.passingMarks || 40}</span>
      ...
      <td className="performance-cell">
        ...
        <div className="performance-stats">
          <div className="stat-item">
            <span className="stat-label">Correct:</span>
            <span className="stat-value correct">{student.correctCount || 0}</span>
          </div>
          <div className="stat-item">
            <span className="stat-label">Wrong:</span>
            <span className="stat-value wrong">{student.wrongCount || 0}</span>
          </div>
        </div>
      </td>
      ...
      <td className="time-cell">
        <div className="time-display">
          ...
          <div className="time-details">
            <div className="time-value">
              {Math.floor((student.timeTaken || 0) / 60)}:
              {((student.timeTaken || 0) % 60).toString().padStart(2, '0')}
            </div>
            <div className="time-label">min:sec</div>
          </div>
        </div>
      ...
      <td className="status-cell">
        ...
        <div className="attempt-date">
          <FaCalendarAlt className="date-icon" />
          {student.attemptedAt ? new Date(student.attemptedAt).toLocaleDateString() : "N/A"}
        </div>
      </td>
...
const TopPerformersTab = React.memo(({
  sortedTopPerformers,
  data,
  topPerformersSort,
  handleTopPerformersSort,
  setSelectedStudent,
  getPerformanceColor,
  getPerformanceLabel,
  stats,
  ...
  <div className="top-performers-container">
      ...
      <div className="performers-header-right">
        <div className="top-stats">
          <div className="top-stat">
            <div className="top-stat-label">Avg. Score (Top 5)</div>
            <div className="top-stat-value">
              {stats?.top5AvgScore?.toFixed(1) || "0.0"}
            </div>
          </div>
          <div className="top-stat">
            <div className="top-stat-label">Highest Score</div>
            <div className="top-stat-value">{analytics?.highestScore || 0}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="top-performers-controls">
      ...
      <div className="results-info">
        <span className="results-count">
          Showing Top 5 of {data.attemptedCount || 0} total participants
        </span>
      </div>
    </div>
      ...
      <div className="performers-grid">
        {sortedTopPerformers.map((student, index) => (
          <TopPerformerCard 
            ...
            getPerformanceLabel={getPerformanceLabel}
            totalStudents={data.attemptedCount || 0}
            quizDuration={data.quiz?.durationMinutes || 60}
          />
        ))}
      </div>
    </div>
      ...
      <div className="comparison-card">
        ...
        <div className="comparison-stats">
          <div className="comparison-stat">
            ...
            <div className="comparison-diff">
              <FaArrowUp className="diff-icon" />
              +{((stats?.top5AvgScore || 0) - (analytics?.averageScore || 0)).toFixed(1)}% vs class avg
            </div>
          </div>
          <div className="comparison-stat">
            <div className="comparison-label">Class Average</div>
          ...
          <div className="comparison-stat">
            <div className="comparison-label">Performance Gap</div>
            <div className="comparison-value">
              {((analytics?.highestScore || 0) - (analytics?.lowestScore || 0)).toFixed(1)}
            </div>
            <div className="comparison-diff">
              Score range in class
            </div>
      ...
      <div className="legend-card">
        ...
        <div className="scale-items">
          <div className="scale-item excellent">
            ...
            <div className="scale-count">
              {analytics?.performanceDistribution?.excellent || 0} students
            </div>
          </div>
          <div className="scale-item good">
            <div className="scale-color"></div>
            ...
            <div className="scale-count">
              {analytics?.performanceDistribution?.good || 0} students
            </div>
          </div>
          <div className="scale-item average">
            <div className="scale-color"></div>
            ...
            <div className="scale-count">
              {analytics?.performanceDistribution?.average || 0} students
            </div>
          </div>
        </div>
      </div>
...
const TopPerformerCard = React.memo(({ student, index, setSelectedStudent, getPerformanceColor, getPerformanceLabel, totalStudents, quizDuration }) => {
  const percentage = parseFloat(student.percentage || 0);
  const performanceColor = getPerformanceColor(percentage);
  const performanceLabel = getPerformanceLabel(percentage);
  
  return (
    <div className="performer-card-modern">
      <div className="performer-medal-section">
        <div className={`medal-display rank-${index + 1}`}>
          {index === 0 && <div className="gold-glow"></div>}
          {index === 1 && <div className="silver-glow"></div>}
        ...
        <div className="profile-avatar-modern">
          ...
          <div className="profile-info-modern">
            ...
            <div className="student-details-modern">
              ...
              <div className="detail-item">
                <FaUserGraduate className="detail-icon" />
                <span>ID: {student.student?.enrollmentNumber || "N/A"}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="performance-metrics-section">
        <div className="score-display-modern">
          <div className="score-main-modern">
            <span className="score-value-modern">{student.score || 0}</span>
            <span className="score-divider-modern">/</span>
            <span className="score-total-modern">{student.totalMarks || 100}</span>
          </div>
          <div className="score-progress-modern">
            <div className="progress-bar-modern">
              <div 
              ...
              <div 
                className="passing-line" 
                style={{ left: `${((student.passingMarks || 40) / (student.totalMarks || 100)) * 100}%` }}
              >
                <div className="line-dot"></div>
                <div className="line-label">Pass</div>
              </div>
            ...
            <div className="progress-labels">
              ...
              <span>{student.totalMarks || 100}</span>
            </div>
          </div>
        </div>
        ...
        <div className="performance-breakdown">
          ...
          <div className="breakdown-item">
            ...
            <div className="time-efficiency">
              <FaClock className="time-icon-modern" />
              <span className="time-value-modern">
                {Math.floor((student.timeTaken || 0) / 60)}:{((student.timeTaken || 0) % 60).toString().padStart(2, '0')}
              </span>
              <span className="efficiency-label">
                ({Math.round(((student.timeTaken || 0) / (quizDuration * 60)) * 100)}% used)
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="stats-section">
        <div className="stats-grid-mini">
          <div className="stat-card-mini correct-stat">
            <FaCheckCircle className="stat-icon-mini" />
            <div className="stat-content-mini">
              <div className="stat-value-mini">{student.correctCount || 0}</div>
              <div className="stat-label-mini">Correct</div>
            </div>
          </div>
          <div className="stat-card-mini wrong-stat">
            <FaTimesCircle className="stat-icon-mini" />
            <div className="stat-content-mini">
              <div className="stat-value-mini">{student.wrongCount || 0}</div>
              <div className="stat-label-mini">Wrong</div>
            </div>
          </div>
          <div className="stat-card-mini rank-stat">
            ...
            <div className="stat-content-mini">
              <div className="stat-value-mini">
                Top {Math.round(((student.rank || 1) / (totalStudents || 1)) * 100)}%
              </div>
              <div className="stat-label-mini">Percentile</div>
            </div>
          </div>
      ...
      <div className="action-section">
        ...
        <div className="attempt-info">
          <FaCalendarAlt className="calendar-icon" />
          <span className="attempt-date-modern">
            {student.attemptedAt ? new Date(student.attemptedAt).toLocaleDateString('en-US', {
              month: 'short',
              day: 'numeric',
              year: 'numeric'
            }) : "N/A"}
...
const AnalyticsTab = React.memo(({ data, departmentColors, getPerformanceColor }) => (
  <div className="analytics-grid">
    <div className="analytics-card">
      <h3><FaChartBar /> Detailed Statistics</h3>
      <div className="analytics-stats">
        <div className="stat-row">
          <span>Total Attempts</span>
          <strong>{data.analytics?.totalAttempts || 0}</strong>
        </div>
        <div className="stat-row">
          <span>Average Score</span>
          <strong>{data.analytics?.averageScore?.toFixed(2) || "0.00"}</strong>
        </div>
        <div className="stat-row">
          <span>Highest Score</span>
          <strong>{data.analytics?.highestScore || 0}</strong>
        </div>
        <div className="stat-row">
          <span>Lowest Score</span>
          <strong>{data.analytics?.lowestScore || 0}</strong>
        </div>
        <div className="stat-row highlight">
          <span>Passing Rate</span>
          <strong className={(data.analytics?.passingRate || 0) >= 50 ? "success" : "danger"}>
            {data.analytics?.passingRate?.toFixed(1) || "0.0"}%
          </strong>
        </div>
      </div>
    </div>
    ...
    <div className="analytics-card">
      <h3><FaUniversity /> Department Analysis</h3>
      <div className="dept-analysis">
        {data.analytics?.departmentStats && Object.entries(data.analytics.departmentStats).map(([dept, count], index) => (
          <div key={dept} className="dept-item">
            <span className="dept-name">{dept}</span>
            <div className="dept-bar">
              <div 
                className="dept-fill" 
                style={{ 
                  width: `${((count || 0) / (data.attemptedCount || 1)) * 100}%`,
                  backgroundColor: departmentColors[index % departmentColors.length]
                }}
              ></div>
            </div>
            <span className="dept-count">{count || 0} students</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="analytics-card full-width">
      ...
      <div className="insights-grid">
        <div className="insight-item excellent">
          ...
          <div className="insight-content">
            <h4>Excellent (90-100%)</h4>
            <p>{data.analytics?.performanceDistribution?.excellent || 0} students</p>
            <div className="insight-percentage">
              {((data.analytics?.performanceDistribution?.excellent || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
            </div>
          </div>
        </div>
        <div className="insight-item good">
          ...
          <div className="insight-content">
            <h4>Good (75-89%)</h4>
            <p>{data.analytics?.performanceDistribution?.good || 0} students</p>
            <div className="insight-percentage">
              {((data.analytics?.performanceDistribution?.good || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
            </div>
          </div>
        </div>
        <div className="insight-item average">
          ...
          <div className="insight-content">
            <h4>Average (50-74%)</h4>
            <p>{data.analytics?.performanceDistribution?.average || 0} students</p>
            <div className="insight-percentage">
              {((data.analytics?.performanceDistribution?.average || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
            </div>
          </div>
        </div>
        <div className="insight-item poor">
          ...
          <div className="insight-content">
            <h4>Poor (25-49%)</h4>
            <p>{data.analytics?.performanceDistribution?.poor || 0} students</p>
            <div className="insight-percentage">
              {((data.analytics?.performanceDistribution?.poor || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
            </div>
          </div>
        </div>
        <div className="insight-item fail">
          ...
          <div className="insight-content">
            <h4>Fail (0-24%)</h4>
            <p>{data.analytics?.performanceDistribution?.fail || 0} students</p>
            <div className="insight-percentage">
              {((data.analytics?.performanceDistribution?.fail || 0) / (data.attemptedCount || 1) * 100).toFixed(1)}%
            </div>
          </div>
        </div>
      </div>
...
const PerformanceTab = React.memo(({ data, attemptedStudents, getPerformanceColor, chartData }) => (
  <div className="performance-container">
    <div className="performance-chart">
      <h3>Score vs Time Taken Analysis</h3>
      <ResponsiveContainer width="100%" height={400}>
      ...
      <div className="comparison-card">
        ...
        <ul className="top-performers">
          {(attemptedStudents || []).slice(0, 5).map((student, index) => (
            <li key={student.attemptId || index}>
              <div className="top-performer-rank">
                <span className={`rank-badge-small rank-${index + 1}`}>
                  #{index + 1}
                </span>
              ...
              <div className="top-performer-info">
                <div className="top-performer-name">{student.student?.name || "Unknown"}</div>
                <div className="top-performer-score">
                  <span className="score-value">{student.score || 0}</span>
                  <span className="score-percentage">({student.percentage || 0}%)</span>
                </div>
              </div>
            </li>
          ))}
...
const StudentModal = React.memo(({ selectedStudent, setSelectedStudent, data, getPerformanceColor, getPerformanceLabel }) => (
  <div className="modal-overlay" onClick={() => setSelectedStudent(null)}>
    <div className="modal-content" onClick={e => e.stopPropagation()}>
      <div className="modal-header">
        <div className="modal-title-section">
      ...
      <div className="modal-body">
        <div className="student-profile-header">
          <div className="profile-avatar-section">
            ...
            <div className="profile-rank-badge">
              <FaCrown className="crown-icon" />
              <span>Rank #{selectedStudent.rank || 1}</span>
            </div>
          </div>
          
          <div className="profile-info-section">
            <div className="profile-name-section">
              <h3>{selectedStudent.student?.name || "Unknown"}</h3>
              <div className="profile-enrollment">
            ...
            <div className="profile-status-section">
              ...
              <div className="performance-category" style={{ color: getPerformanceColor(parseFloat(selectedStudent.percentage || 0)) }}>
                {getPerformanceLabel(parseFloat(selectedStudent.percentage || 0))} Performance
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="performance-summary-cards">
          <div className="summary-card score-card">
            ...
            <div className="summary-content">
              <div className="summary-label">Quiz Score</div>
              <div className="summary-value">
                <span className="score-main">{selectedStudent.score || 0}</span>
                <span className="score-divider">/</span>
                <span className="score-total">{selectedStudent.totalMarks || 100}</span>
              </div>
              <div className="summary-percentage">
          ...
          <div className="summary-card time-card">
            ...
            <div className="summary-content">
              <div className="summary-label">Time Taken</div>
              <div className="summary-value">
                {Math.floor((selectedStudent.timeTaken || 0) / 60)}:
                {((selectedStudent.timeTaken || 0) % 60).toString().padStart(2, '0')}
              </div>
              <div className="summary-detail">
                {Math.round(((selectedStudent.timeTaken || 0) / ((data.quiz?.durationMinutes || 60) * 60)) * 100)}% of total time
              </div>
            </div>
          </div>
          ...
          <div className="summary-card comparison-card">
            ...
            <div className="summary-content">
              ...
              <div className="summary-detail">
                Top {Math.round(((selectedStudent.rank || 1) / (data.attemptedCount || 1)) * 100)}% of class
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="detailed-info-grid">
          <div className="info-section">
            ...
            <div className="section-content">
              <div className="info-row">
                <span className="info-label">Email Address</span>
                <span className="info-value">{selectedStudent.student?.email || "N/A"}</span>
              </div>
              <div className="info-row">
                <span className="info-label">Mobile Number</span>
                <span className="info-value">{selectedStudent.student?.mobileNumber || "N/A"}</span>
              </div>
              <div className="info-row">
                <span className="info-label">Gender</span>
                <span className="info-value">
          ...
          <div className="info-section">
            ...
            <div className="section-content">
              ...
              <div className="info-row">
                <span className="info-label">Course & Semester</span>
                <span className="info-value">{selectedStudent.student?.course || "N/A"} - Semester {selectedStudent.student?.semester || "N/A"}</span>
              </div>
              <div className="info-row">
                <span className="info-label">Group</span>
          ...
          <div className="info-section">
            ...
            <div className="section-content">
              <div className="info-row">
                <span className="info-label">Attempted On</span>
                <span className="info-value">
                  {selectedStudent.attemptedAt ? new Date(selectedStudent.attemptedAt).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
              ...
              <div className="info-row">
                <span className="info-label">Attempt Time</span>
                <span className="info-value">
                  {selectedStudent.attemptedAt ? new Date(selectedStudent.attemptedAt).toLocaleTimeString() : "N/A"}
                </span>
              </div>
              <div className="info-row">
                <span className="info-label">Status</span>
          ...
          <div className="info-section">
            ...
            <div className="section-content">
                ...
                <div className="progress-comparison">
                  <div className="progress-bar-container">
                    <div className="progress-bar">
                      <div 
                        className="progress-fill score-progress" 
                        style={{ width: `${((selectedStudent.score || 0) / (selectedStudent.totalMarks || 100)) * 100}%` }}
                      ></div>
                    </div>
                    <div className="progress-labels">
                      <span>0</span>
                      <span className="passing-mark" style={{ left: `${((selectedStudent.passingMarks || 40) / (selectedStudent.totalMarks || 100)) * 100}%` }}>
                        Pass: {selectedStudent.passingMarks || 40}
                      </span>
                      <span>{selectedStudent.totalMarks || 100}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="info-row">
                <span className="info-label">Performance Category</span>
                <span className="info-value performance-category-badge" style={{ backgroundColor: getPerformanceColor(parseFloat(selectedStudent.percentage || 0)) }}>
                  {getPerformanceLabel(parseFloat(selectedStudent.percentage || 0))}
                </span>
              </div>
              <div className="info-row">
                <span className="info-label">Class Comparison</span>
                <span className="info-value">
                  Better than {Math.round(((data.attemptedCount || 1) - (selectedStudent.rank || 1)) / (data.attemptedCount || 1) * 100)}% of class
                </span>
              </div>
            </div>
          </div>
